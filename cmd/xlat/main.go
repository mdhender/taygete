...
// Package main implements tools to translate Olympia files to JSON.
package main
...
func main() {
	...
	cmdRoot := &cobra.Command{
		Use:           "xlat",
		Short:         "taygete file translator",
		Long:          `Translate taygete files to JSON.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func cmdGates() *cobra.Command {
	addFlags := func(cmd *cobra.Command) error {
		return nil
	}
	var cmd = &cobra.Command{
		Use:   "gates",
		Short: "translate gate file",
		Args:  cobra.MinimumNArgs(1), // gate files to translate
		RunE: func(cmd *cobra.Command, args []string) error {
			var gfs = []GateFile{}
			for _, input := range args {
				gf, err := loadGateFile(input)
				if err != nil {
					logger.Error("gates: marshal", "err", err)
					return err
				}
				gfs = append(gfs, gf)
			}
			buf, err := json.MarshalIndent(gfs, "", "  ")
			if err != nil {
				logger.Error("gates: marshal", "err", err)
				return err
			}
